...
dropbearlogfile="/jffs/addons/iocmon.d/dropbear_attempts.log"  # every raw dropbear "Bad password attempt" syslog
                                 # line, trimmed to $logsize like $logfile - kept separately so low-and-slow
                                 # brute-force traffic (below the alert threshold) is still visible for manual
                                 # review, not just the ones that actually crossed authfailthreshold/authslowthreshold
...
# -------------------------------------------------------------------------------------------------------------------------
# dedupindicators collapses duplicate "indicator|source|family" lines in $1 down to one line per unique
# indicator, comma-joining every distinct source and family that reported it.
...
# -------------------------------------------------------------------------------------------------------------------------
# realdirpath resolves $1 to its canonical absolute form
...
# -------------------------------------------------------------------------------------------------------------------------
# autostart adds/removes the services-start hook that launches IOCMON's background SCREEN monitor on reboot,
# mirroring TAILMON's reboot-protection convention.
...
autostart()
{
  ...
  if [ "$autostart" -eq 1 ]; then
    echo "sh $apppath -screen -now # iocmon-autostart" >> /jffs/scripts/services-start
  fi
}
...
alertviewmode="ioc"             # which log the "Recent ..." dashboard block and the (V) hotkey show - "ioc"
                                 # or "dropbear", toggled by the (D)/(O) hotkeys
