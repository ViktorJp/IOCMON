...
#   /tmp/mnt/<extdrivelabel>/iocmon.d/state/fs_last_new.txt: filenames added, most recent scan only (overwritten)
#   /tmp/mnt/<extdrivelabel>/iocmon.d/state/fs_last_modified.txt: filenames modified, most recent scan only
#   /tmp/mnt/<extdrivelabel>/iocmon.d/state/fs_last_deleted.txt: filenames deleted, most recent scan only
#   /tmp/mnt/<extdrivelabel>/iocmon.d/state/fs_last_permchanged.txt: filenames with a permission-only change
...
#   (a) acknowledge the persistent red alert banner            feed indicator, to prove the alert pipeline works
#
# ============================================================================================================================
...
dlverpath="/jffs/addons/iocmon.d/version.txt"
bverpath="/jffs/addons/iocmon.d/beta.txt"
logfile="/jffs/addons/iocmon.d/iocmon.log"
updatingfile="/jffs/addons/iocmon.d/updating.txt"
dropbearlogfile="/jffs/addons/iocmon.d/dropbear_attempts.log"

# Repo used for self-update checks.
iocmonrepostable="https://raw.githubusercontent.com/IOCMON/main"
iocmonrepobeta="https://raw.githubusercontent.com/IOCMON/develop"
...
enablednswatch=0                # tail dnsmasq log for domain matches - off by default: turning it on from
                                 # Advanced Settings item 4 is what prompts to enable dnsmasq query logging too,
                                 # so a fresh install doesn't start in an "on but not actually watching yet" state
dnsexceptions=""                # space-separated domains that match a feed but never alert/email - just an
                                 # INFO log line - for known false positives (e.g. ThreatFox's url-host
                                 # extraction flagging a legit CDN/service that merely hosted a malicious URL)
enablednstunnel=0               # behavioral heuristic, not feed-based - off by default since it can false-
                                 # positive on legitimate high-subdomain-churn services; unlike the feed-match
                                 # DNS watch, this looks for tunneling/exfiltration *patterns* in query volume/
                                 # shape. Reuses dnsexceptions (matched against the base domain) to quiet
                                 # known-legitimate high-churn services once spotted.
dnstunnelsubthreshold=20        # distinct subdomains under one base domain from one source IP, within a
                                 # single check tick, to flag as possible DNS tunneling/exfiltration
dnstunnelnamelen=60             # a single query name at or above this length (chars) is flagged on its own,
                                 # regardless of repetition - tunneling tools typically stuff many encoded
                                 # bytes into one label; ordinary hostnames essentially never get this long
...
authslowthreshold=15            # same-source-IP login failures across that whole window to trigger a
                                 # "sustained" alert - catches an attacker pacing under authfailthreshold
...
cronexceptions=""                # newline-separated exact cron-line matches (NOT space-separated like the
                                 # other exception lists - a cron entry's own schedule/command text always
                                 # contains spaces) that never alert even when checkcronbaseline/checkentwarecron
                                 # see them appear as "new" - for legitimate cron entries that get removed and
                                 # re-added by another script/addon on their own schedule (e.g. amtm's LED
                                 # on/off scheduler), which otherwise look exactly like injected persistence
                                 # every single time they're re-added
enablenvramwatch=1              # diff a hardcoded security-relevant NVRAM watchlist every tick - see
                                 # $nvramwatchvars below; cheap (no filesystem I/O), so no cadence gate needed
enablefwrulediff=1              # alert on unexpected new port-forward/DMZ/UPnP NAT rules - see
                                 # checkfirewallrules(); shares fsintegritycheck's scan cadence, same as
                                 # checkcronbaseline/checkentwarecron
enablefsdeletionwatch=1         # alert when a file DISAPPEARS from one of the same critical paths that already
                                 # alert on an addition/edit regardless of hash match (.ssh/, a known startup
                                 # script directly in /jffs/scripts, a *.conf.add in /jffs/configs) - deliberately
                                 # NOT "any deleted file", same noise-avoidance scoping as those existing checks
enablepermwatch=0               # detect a WRITE or EXECUTE bit appearing on an already-known file whose
                                 # content/mtime is otherwise unchanged - a plain `chmod +x` never touches mtime,
                                 # so a payload dropped without +x and made executable later would otherwise
                                 # never trip the "new file arrived executable" heuristic OR the mtime-based
                                 # modified-file check. Off by default, unlike the other watches above: unlike a
                                 # cheap diff against a small list, this needs one `ls -l` PROCESS FORK per
                                 # watched file, every scan, to read the write bit correctly (see iswritablebit's
                                 # own comment for why `[ -w file ]` can't be used as root) - on a watch list
                                 # that includes /opt/bin and /opt/sbin (the shipped default), that can mean
                                 # hundreds of forks per scan on a router with a lot of Entware packages
                                 # installed. Turn on only after checking how many files are actually in your
                                 # watched folders, e.g. `find /opt/bin /opt/sbin -type f | wc -l`.
...
##-------------------------------------------##
## Borrowed from ExtremeFiretop / TAILMON     ##
##-------------------------------------------##
...
progressbaroverride()
{
  ...
  if [ $1 -eq -1 ]; then
    ...
    if [ "$5" == "Standard" ]; then
      ...
      drawprogressprompt "${InvGreen} ${CClear} ${CWhite}${InvDkGray}${AltNumPadded}${4} / ${progrPadded}%${CClear} [${CGreen}c${CClear}=Config] [${CGreen}f${CClear}=Feeds] [${CGreen}i${CClear}=FS Integrity] [${CGreen}t${CClear}=Test] [${CGreen}a${CClear}=Ack] [${CGreen}l${CClear}=Logs] [${CGreen}e${CClear}=Exit]${CClear}" "[Key+Enter?  ]"
    fi
  fi
  ...
  if readmenucommand; then
      ...
      case $key_press in
          ...
          [Ii]) forcefsintegrity;;
          [Vv]) vioclog;;
          [Dd]) alertviewmode="dropbear"; timer=$timerloop;;
          [Oo]) alertviewmode="ioc"; timer=$timerloop;;
          [Tt]) testdetection;;
          [Aa]) acknowledgealert;;
          [Ll]) vlogs;;
...
# -------------------------------------------------------------------------------------------------------------------------
# lastalertsummary reads the most recent "IOC match" line out of the log and echoes "HH:MM kind: indicator",
# or "none yet" if nothing has fired.
...
vsetup()
{
  while true; do
    ...
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(5)${CClear} : Advanced Settings${CClear}"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(6)${CClear} : Uninstall IOCMON${CClear}"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite} | ${CClear}"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(e)${CClear} : Exit${CClear}"
    echo -e "${InvGreen} ${CClear}"
    echo -e "${InvGreen} ${CClear}${CDkGray}-----------------------------------------------------------------------------------------------------------------------------------------${CClear}"
    echo ""
    read -p "Please select? (1-6, e=Exit): " selsetup
    case "$selsetup" in
      1) selectextdrive ;;
      2) vfeedsources ;;
      3) forcefeeds ;;
...
veditlist()
{
  local varname="$1" title="$2" list count entry idx sel newval delnum newlist
  ...
  while true; do
    ...
    case "$sel" in
      [Aa])
        read -p "New entry (absolute path): " newval
        if [ -n "$newval" ]; then
...
# -------------------------------------------------------------------------------------------------------------------------
# vcronexceptions manages $cronexceptions - newline-separated EXACT cron-line matches
...
# -------------------------------------------------------------------------------------------------------------------------
# vadvancedbruteforce: dropbear/httpd login-failure detection, both the per-tick burst check and the sliding-
# window sustained check.
...
# -------------------------------------------------------------------------------------------------------------------------
# vadvancedfilesystem: the poll-based baseline diff (checkfsintegrity), its watched/excluded paths, hashing
# limit, quarantine behavior, and the cron-tampering diff (checkcronbaseline/checkentwarecron) that shares its
# scan cadence.
...
vadvancedfilesystem()
{
  while true; do
    ...
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}( 4)${CClear} : "; padright "Folder names to skip while watching" 68; echo -e ": ${CGreen}$(echo "$fswatchexclude" | wc -w | tr -d ' ')${CClear} configured"
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}( 5)${CClear} : "; padright "Skip hashing files larger than this, in bytes" 68; echo -e ": ${CGreen}$fsmaxhashsize${CClear}"
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}( 6)${CClear} : "; padright "Auto-quarantine files matching a known-malware hash (strips +x," 68; echo -e ": $(booleantoyesno "$enablequarantine")"
    echo -e "${InvGreen} ${CClear}         renames to <file>.iocmon-quarantine, never deletes)"
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}( 7)${CClear} : "; padright "Alert on unexpected new scheduled tasks (cru/Entware cron)" 68; echo -e ": $(booleantoyesno "$enablecrondiff")"
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}( 8)${CClear} : "; padright "Alert when a file is DELETED from a critical path" 68; echo -e ": $(booleantoyesno "$enablefsdeletionwatch")"
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}( 9)${CClear} : "; padright "Alert when WRITE/EXECUTE permission is added with no changes" 68; echo -e ": $(booleantoyesno "$enablepermwatch")"
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(10)${CClear} : "; padright "Cron entries excepted from unexpected-new-entry alerts" 68; echo -e ": ${CGreen}$([ -z "$cronexceptions" ] && echo 0 || printf '%s\n' "$cronexceptions" | wc -l | tr -d ' ')${CClear} configured"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite} | ${CClear}"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(e)${CClear} : Return to Advanced Settings${CClear}"
    echo -e "${InvGreen} ${CClear}"
    echo -e "${InvGreen} ${CClear}${CDkGray}-----------------------------------------------------------------------------------------------------------------------------------------${CClear}"
    echo ""
    read -p "Please select? (1-10, e=Exit): " sel
    case "$sel" in
      ...
      3) veditlist fswatchdirs "Watched Folders" ;;
      4) veditlist fswatchexclude "Excluded Folder Names" ;;
      5) echo -e "Current: ${CGreen}${fsmaxhashsize}${CClear}"
         read -p "New max hash size in bytes (>=1, blank to keep current): " val
         [ -n "$val" ] && { validateint "$val" 1 && fsmaxhashsize="$val" && saveconfig; } ;;
      6) togglesetting enablequarantine ;;
      7) togglesetting enablecrondiff ;;
      8) togglesetting enablefsdeletionwatch ;;
      9) togglesetting enablepermwatch ;;
      10) vcronexceptions ;;
      [Ee]) break ;;
      *) ;;
    esac
...
# -------------------------------------------------------------------------------------------------------------------------
# vadvancednetwork: conntrack (live traffic against known-malicious IPs) and NVRAM (router config state, mostly SSH/Telnet
# exposure and WAN DNS) are different detection domains, grouped together here as the router/network-level watches
...
# -------------------------------------------------------------------------------------------------------------------------
# vfeedsources is the Feed Sources & API Keys submenu: per-source enable toggles and the optional ThreatFox
# API key. ThreatFox falls back to the free unauthenticated CSV export whenever no key is set.
...
# -------------------------------------------------------------------------------------------------------------------------
# trimlogfile is the same trim-to-N-lines logic as trimlogs, generalized to any file/size pair - used by
# checkauth() to keep dropbearlogfile self-bounding without the $logfile-specific INFO log line trimlogs writes.
...
# -------------------------------------------------------------------------------------------------------------------------
# fetchthreatfox fetches ThreatFox separately from fetchfeedsource because it has two very different transports:
# the authenticated POST /api/v1/ JSON query (used when an API key is configured and jq is available to parse
# the response) and the free unauthenticated CSV export (used otherwise).
...
# -------------------------------------------------------------------------------------------------------------------------
# dedupindicators collapses duplicate "indicator|source|family" lines in $1 down to one line per unique indicator
...
# -------------------------------------------------------------------------------------------------------------------------
# feedsourcecounts prints a compact "source=N source=N" breakdown across all three canonical files (Spamhaus
# DROP/EDROP combined into one "spamhaus" count, since they're the same feed split across two URLs).
...
# -------------------------------------------------------------------------------------------------------------------------
# _DownloadCEMLibraryFile_/_SendEMailNotification_ are TAILMON's shared AMTM email integration, adapted verbatim
# for IOCMON branding.
...
# -------------------------------------------------------------------------------------------------------------------------
# checkconntrack reads /proc/net/nf_conntrack, extracts the originating (src) and destination (dst) IP of each
# tracked connection's first (original-direction) src=/dst= pair
...
# -------------------------------------------------------------------------------------------------------------------------
# hashmatchcheck computes whichever of md5/sha1/sha256 are available for one file and checks all of them
# against feeds/hashes.txt in one pass
...
checkfsintegrity()
{
  local hashesfile="" excludeargs="" pruneargs="-path $addonsdir -o -path $CUSTOM_EMAIL_LIBDir" d ex realdir
  ...
  local dircount=0 watcheddirs="" dirfilecount path filesize isexec pex pwr permdesc permtab
  ...
  for ex in $fswatchexclude; do
    excludeargs="$excludeargs ! -path */$ex/*"
  done

  [ -n "$iocmonroot" ] && pruneargs="-path $iocmonroot -o $pruneargs"
  ...
  for d in $fswatchdirs; do
    ...
    dirfilecount="$(find "$realdir" -xdev \( $pruneargs \) -prune -o -type f ! -name '*.iocmon-quarantine' $excludeargs -print 2>>"$scanerrors" | tee -a "$newmanifest" | wc -l | tr -d ' ')"
    watcheddirs="${watcheddirs}${realdir}: ${dirfilecount} files\n"
    dircount=$((dircount+1))
  done

  sort "$newmanifest" -o "$newmanifest"
  fsdircount=$dircount
  ...
  if [ -z "$dircount" ] || [ "$dircount" -eq 0 ]; then
    ...
    rm -f "$newmarker"
    : > "$lastnewfile"; : > "$lastmodfile"; : > "$lastdelfile"; : > "$lastpermfile"
    return
  fi
  ...
  if [ -f "$marker" ]; then
    for d in $fswatchdirs; do
      [ -d "$d" ] || continue
      realdir="$(realdirpath "$d")"
      find "$realdir" -xdev \( $pruneargs \) -prune -o -type f ! -name '*.iocmon-quarantine' $excludeargs -newer "$marker" -print 2>/dev/null >> "$modpathsfile"
    done
  fi
  ...
  cp "$newpathsfile" "$lastnewfile"
  cp "$modpathsfile" "$lastmodfile"
  cp "$delpathsfile" "$lastdelfile"
  cut -f1 "$permchangesfile" > "$lastpermfile"

  {
    printf 'New files list: %s (%s entries)\n' "$lastnewfile" "$fsnewcount"
    printf 'Modified files list: %s (%s entries)\n' "$lastmodfile" "$fsmodcount"
    printf 'Deleted files list: %s (%s entries)\n' "$lastdelfile" "$fsdelcount"
    printf 'Permission-changed files list: %s (%s entries)\n' "$lastpermfile" "$fspermcount"
  } >> "$scansummary"
  ...
  if [ ! -s "$newpathsfile" ] && [ ! -s "$modpathsfile" ] && [ ! -s "$delpathsfile" ] && [ ! -s "$permchangesfile" ]; then
    rm -f "$newpathsfile" "$modpathsfile" "$delpathsfile" "$permchangesfile"
    return
  fi

  { sed 's/^/NEW:/' "$newpathsfile"; sed 's/^/MOD:/' "$modpathsfile"; } | while IFS=: read -r changetype path; do
    [ -f "$path" ] || continue
    filesize="$(wc -c < "$path" 2>/dev/null)"
...
# incronexceptionlist is an exact whole-line match against $cronexceptions

incronexceptionlist()
{
  local entry="$1"
  [ -z "$cronexceptions" ] && return 1
  printf '%s\n' "$cronexceptions" | grep -qxF "$entry"
}
...
{
  clear
  echo -e "${CGreen}[Forcing Filesystem-Integrity Check]${CClear}"
...
autostart()
{
  # Clean up anyone who still has the old, broken services-start-based hook installed.
  if [ -f /jffs/scripts/services-start ]; then
