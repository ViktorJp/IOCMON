...
enablefeodo=1
enableurlhaus=1
enablednswatch=0                # tail dnsmasq log for domain matches
dnsexceptions=""                # space-separated domains that match a feed but never alert/email
enablednstunnel=0               # behavioral heuristic, not feed-based
dnstunnelsubthreshold=20        # distinct subdomains under one base domain from one source IP
dnstunnelnamelen=60
...
cronexceptions=""
enablenvramwatch=1              # diff a hardcoded security-relevant NVRAM watchlist every tick
enablefwrulediff=1              # alert on unexpected new port-forward/DMZ/UPnP NAT rules
enablefsdeletionwatch=1         # alert when a file DISAPPEARS from one of the same critical paths
enablepermwatch=0               # detect a WRITE or EXECUTE bit appearing on an already-known file
...
vadvancedfilesystem()
{
  while true; do
    ...
    echo -en "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(11)${CClear} : "; padright "File extensions excluded from every check (new/mod/del/perm)" 68; echo -e ": ${CGreen}$(echo "$fsexcludeext" | wc -w | tr -d ' ')${CClear} configured"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite} | ${CClear}"
    echo -e "${InvGreen} ${CClear} ${InvDkGray}${CWhite}(e)${CClear} : Return to Advanced Settings${CClear}"
    echo -e "${InvGreen} ${CClear}"
    echo -e "${InvGreen} ${CClear}${CDkGray}-----------------------------------------------------------------------------------------------------------------------------------------${CClear}"
    echo ""
    read -p "Please select? (1-11, e=Exit): " sel
    case "$sel" in
      ...
      10) vcronexceptions ;;
      11) veditlist fsexcludeext "Excluded File Extensions" "a file extension, e.g. .log (leading dot optional)" ;;
      [Ee]) break ;;
      *) ;;
    esac
  done
...
# -------------------------------------------------------------------------------------------------------------------------
# fspathexcluded is drainfsevents' equivalent of checkfsintegrity's own find-argument exclusions
...
checkfsintegrity()
{
  ...
  for ex in $fsexcludeext; do
    ...
    excludeargs="$excludeargs ! -name *$ex"
  done
