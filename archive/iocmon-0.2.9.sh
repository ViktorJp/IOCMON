...
checkfsintegrity()
{
  ...
  if [ ! -s "$oldmanifest" ]; then
    ...
    while IFS= read -r path; do
      [ -z "$path" ] && continue
      scancounter=$((scancounter+1))
      [ $((scancounter % 25)) -eq 0 ] && fsscanprogress "Building initial size baseline" "$scancounter" "$fsscannedcount"
      sz="$(wc -c < "$path" 2>/dev/null)"
      [ -z "$sz" ] && sz=0
      printf '%s\t%s\n' "$path" "$sz" >> "$newsizebaseline"
    done < "$oldmanifest"
  ...
  {
    printf '[%s1%s] New files list: %s (%s entries)\n' "$CGreen" "$CClear" "$lastnewfile" "$(wc -l < "$lastnewfile" | tr -d ' ')"
    printf '[%s2%s] Modified files list: %s (%s entries)\n' "$CGreen" "$CClear" "$lastmodfile" "$(wc -l < "$lastmodfile" | tr -d ' ')"
    printf '[%s3%s] Deleted files list: %s (%s entries)\n' "$CGreen" "$CClear" "$lastdelfile" "$(wc -l < "$lastdelfile" | tr -d ' ')"
    printf '[%s4%s] Permission-changed files list: %s (%s entries)\n' "$CGreen" "$CClear" "$lastpermfile" "$(wc -l < "$lastpermfile" | tr -d ' ')"
  } >> "$scansummary"
...
# -------------------------------------------------------------------------------------------------------------------------
# checkcronbaseline alerts on any cru entry that appeared since the last check=. incronexceptionlist is an exact whole-line 
# match against $cronexceptions
