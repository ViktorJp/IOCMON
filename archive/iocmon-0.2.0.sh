...
progressbaroverride()
{
  ...
  if [ $1 -eq -1 ]; then
    ...
    if [ "$5" == "Standard" ]; then
      ...
      drawprogressprompt "${InvGreen} ${CClear} ${CWhite}${InvDkGray}${AltNumPadded}${4} / ${progrPadded}%${CClear} [${CGreen}c${CClear}=Config] [${CGreen}f${CClear}=Feeds] [${CGreen}i${CClear}=FS Integrity] [${CGreen}v${CClear}=IoC Log] [${CGreen}t${CClear}=Test] [${CGreen}a${CClear}=Ack] [${CGreen}l${CClear}=Logs] [${CGreen}e${CClear}=Exit]${CClear}" "[Key+Enter?  ]"
    fi
  fi
...
# -------------------------------------------------------------------------------------------------------------------------
# normalizefeeds is the normalize+atomic-swap half of the pipeline: it re-tags every source's raw cache into
# the canonical "indicator|source|malware_family" format, deduplicates/merges across sources, and rebuilds
# feeds/ips.txt, domains.txt and hashes.txt from scratch each cycle, then swaps each into place with mv.
...
checkauth()
{
  ...
  local nowepoch cutoff dropbearips httpdips
  nowepoch=$(date +%s)
  cutoff=$((nowepoch - authslowwindowhrs * 3600))

  dropbearips="$(tail -n "$newlines" "$synclog" | grep -F 'dropbear' | grep -F 'Bad password attempt' | \
...
checkfsintegrity()
{
  ...
  local dircount=0 watcheddirs="" dirfilecount path filesize isexec

  fsdircount=0; fsscannedcount=0; fsnewcount=0; fsmodcount=0
  ...
  echo -e "$(date +'%b %d %Y %X') $(nvram get lan_hostname) IOCMON[$$] - INFO: Filesystem-integrity scan complete: $fsscannedcount files scanned across $dircount directories; $fsnewcount new and $fsmodcount modified file(s) checked against known-malware hashes." >> "$logfile"

  if [ ! -s "$newpathsfile" ] && [ ! -s "$modpathsfile" ]; then
    rm -f "$newpathsfile" "$modpathsfile"
    return
  fi
...
forcefsintegrity()
{
  ...
  if [ -z "$stateroot" ]; then
    ...
    echo -e "  ${CGreen}*${CClear} Found ${CGreen}${fsnewcount}${CClear} new file(s) and ${CGreen}${fsmodcount}${CClear} modified file(s) since the last scan"
    echo ""
...
{
  resolvestateroot
  if [ -z "$stateroot" ] || [ ! -s "$stateroot/ioc_alerts.log" ]; then
    clear
    echo -e "${CYellow}No IoC detections have been logged yet.${CClear}"
    echo ""
    read -rsp $'Press any key to continue...\n' -n1 key
  else
    export TERM=linux
    nano +999999 --linenumbers "$stateroot/ioc_alerts.log"
  fi
  timer=$timerloop
}

# -------------------------------------------------------------------------------------------------------------------------
# testdetection picks one real indicator at random out of the currently loaded feeds and runs it through the exact same
# raisealert path a genuine detection would use (log line, permanent IOC log entry, red banner, AMTM email if enabled) -
# clearly labeled as simulated throughout so it's never mistaken for a real compromise.
...
while true; do
  ...
  if [ "$enablefsintegrity" -eq 1 ] && [ "$fsintegrityranthiscycle" -eq 1 ]; then
    blanklineguard
    echo -e "  ${CGreen}*${CClear} Hashed/scanned ${CGreen}${fsscannedcount}${CClear} local file(s) across ${CGreen}${fsdircount}${CClear} watched directories for unauthorized changes (${CGreen}${fsnewcount}${CClear} new, ${CGreen}${fsmodcount}${CClear} modified)"
    scanbulletprinted=1
    sleep 1
  fi
  ...
  echo -e "${InvGreen} ${CClear} ${CWhite}Recent IoC Detections${CClear} (kept indefinitely - press (V) to view the full log in nano)"
